...
error *newErrorFunc(int errV, const const char *func, const char *where, const char *text, error *linkTo, error *addErr)
{
   char msg[TXT_SZ];

   sprintf(msg, "%s%s", func, where);
   return newError(errV, msg, text, linkTo, addErr);
}
