...
double *readASCII(char *filename, int *fnx, int *fny, sm2_error **err) {
  ...
  testErrorRetVA(n!=2, io_file, "%d elements read from file %s, expected %d",
		 *err, __LINE__, NULL, n, filename, 2);
...
void* calloc_err(size_t nl, size_t sz1, error **err)
{
  ...
  res = calloc(nl,sz1);
  testErrorRetVA(res==NULL, err_allocate, "Cannot allocate %zu objects of %zu bytes (=%zu total)",
...
void *realloc_err(void *ptr, size_t sz, error **err)
{
   ...
   res = realloc(ptr, sz);
   testErrorRetVA(ptr==NULL, err_allocate, "Cannot reallocate %zu bytes", *err, __LINE__, NULL, sz);
   testErrorRetVA(res==NULL, err_allocate, "Cannot reallocate %zu bytes", *err, __LINE__, NULL, sz);
...
double **calloc_array_double(int N, int m, error **err)
{
   double **res;
   int i;
   ...
   res = malloc_err(sizeof(double*) * N, err);
   forwardError(*err, __LINE__, NULL);
   for (i=0; i<N; i++) {
      res[i] = calloc_err(m, sizeof(double), err);
      forwardError(*err, __LINE__, NULL);
   }
  ...
  *nlines = 0;
  while(feof(fp)==0) {
    ...
    while (j<cz) {
      ...
      testErrorRetVA(sscanf(buffer+j0, prs, res +i*sz)!=1, err_io, "Cannot read file '%s' at index %zu",
  ...
  fclose(fp);
  if (clip==1)
    testErrorRetVA(i!=*n, err_io, "Not enough data in file '%s' (got %zu entries, expected %zu)\n",
...
void* read_bin_vector(const char* fnm, size_t n, error **err) {
  ...
  forwardError(*err,__LINE__,NULL);
  rr=fread(res, 1, n, ff);
  testErrorRetVA(rr!=n, io_file, "File '%s' does not contain enough data (Expected %zu bytes, got %zu)",
...
void* read_bin_list(const char* fnm, size_t *n, error **err) {
  ...
  *n = stst.st_size;
  ...
  testErrorRetVA(rr!=*n,io_file,
		 "File '%s' does not contain enough data (Expected %zu bytes, got %zu)",
		 *err, __LINE__, NULL, fnm, *n, rr);
  fclose(ff);
  return res;
}
...
void write_bin_vector(void* buf, const char* fnm, size_t n, error **err) {
  ...
  testErrorRetVA(rr!=n,io_file,
		 "Cannot write data in file '%s' (Expected %zu bytes, got %zu)",
