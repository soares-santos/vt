...
/* New in v1.7: Removed 'newErrorVA', avoid unsafe variable arguments lists */
error* newErrorFunc(int errV, const char *func, const char *where, const char *text, error *linkTo, error *addErr);
error* newError(int errV, const char* where, const char* text, error* linkTo, error* addErr);
...
/* New v1.7: removed calls to newErrorVA */
#define addErrorVA(errV, txt, prev, li, ...)   prev; char messageVA_add[TXT_SZ]; sprintf(messageVA_add, txt, __VA_ARGS__); prev = addError(errV, messageVA_add, prev, li)

#define someError(errV,txt,prev,next,li)       newErrorFunc(errV, __func__, "("__FILE__":"#li")", txt, prev, next)
#define topError(errV,txt,next,li)             someError(errV,txt,NULL,next,li)
#define oneError(errV,txt,li)                  oneError(errV,txt,NULL,NULL,li)
#define addError(errV,txt,prev,li)             someError(errV,txt,prev,NULL,li)
...
// tests 
#define testError(test,error_type,message,err,line)  { \
  if (test) { \
    err = addError(error_type, message, err, line); \
  } \
}

#define testErrorVA(test,error_type,message,err,line,...) {  \
  if (test) {                                                \
    char messageVA[TXT_SZ]; \
    sprintf(messageVA, message, __VA_ARGS__); \
    err = addError(error_type, messageVA, err, line); \
  }                                                          \
}

// New in v1.7: avoid unsafe variable arguments lists */
#define testErrorVA_OLD(test,error_type,message,err,line,...) {  \
  if (test) {                                                \
    err = addErrorVA(error_type, message, err, line, __VA_ARGS__); \
  }                                                          \
}

#define testErrorRet(test,error_type,message,err,line,return_value) { \
  testError(test,error_type,message,err,line);                \
